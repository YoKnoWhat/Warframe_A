...
public:
	 ...
	 * @param	SegmentPoint		Cover point to be decided.
	 * @param	TraceVector         Which direction should we cover from?
	 * returns	true if cover point is valid.
	 */
	bool IsProvidingCover(UWorld* World, const FVector SegmentPoint, const FVector SegmentDirection, const FVector TraceVector, const float StartHeight, const float EndHeight);

	/**
	 * Return true if is valid to lean to the specified direction.
	 * @param	FirstTestDirection	Direction to lean to.
	 * returns	true if cover point is valid.
	 */
